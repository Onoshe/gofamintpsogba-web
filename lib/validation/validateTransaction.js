...
function validateTransactions(forms, chartOfAccounts, personalAccounts, controlAcctsCode, formType, bookLoan) {
   ...
   const cusPref = "C";
    ...
    for (let i = 0; i < updatedForms.length; i++) {
      ...
      //debitAccount or creditAccount should not be an InventoryControl account. This can happen on posting upload
      ...
      //First check if debitSub or creditSub exist for control account
      ...
      if(acctDr.typeCode == receivables || acctDr.typeCode == payables || acctDr.typeCode == inventoryControl){
        if(!debitSub){
          return {
            error: true,
            errorType: 'EMPTY_VALUE',
      ...
      if(acctCr.typeCode == receivables || acctCr.typeCode == payables || acctCr.typeCode == inventoryControl){
        if(!creditSub){
          return {
            error: true,
            errorType: 'EMPTY_VALUE',
      ...
      //Validate bookLoan
      if(bookLoan){
        if(!parseInt(form.dueDate))
          return {
            error: true,
            errorType: 'USE_MSG',
            rowIndex: i,
            key:'dueDate',
            title:'Pleae enter due date on row '+ (i+1)+', or uncheck Book a Loan',
          };
           //Debit account must be a Customer account for it to be a loan account
            const acctDr = chartOfAccounts.find((dt)=> dt.accountCode == debitAccount);
            if(!['accountReceivables', 'accountReceivableControl', 'otherCurrentAsset'].includes(acctDr.typeName)){
              return {
                error: true,
                errorType: 'USE_MSG',
                rowIndex: i,
                title:'Debit account must be a Personal current asset account to qualify as a loan account'+acctDr.typeName,
                key:"accountCode"
              };
            }
      }
