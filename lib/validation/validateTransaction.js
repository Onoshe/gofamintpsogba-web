...
function validateTransactions(forms, chartOfAccounts, personalAccounts, controlAcctsCode, formType) {
   ...
   const cusPref = "C";
    ...
    for (let i = 0; i < updatedForms.length; i++) {
      ...
      let coaStrCredit = {};
      if(debitAccount){
        const acct = chartOfAccounts.find((dt)=> parseInt(dt.accountCode) == parseInt(debitAccount));
        coaStrDebit = {typeCode: acct?.typeCode, typeName:acct?.typeName};
      }
      if(creditAccount){
        const acct = chartOfAccounts.find((dt)=> parseInt(dt.accountCode) == parseInt(creditAccount));
      ...
      if (
        !chartOfAccounts.some((account) => parseInt(account.accountCode) === parseInt(debitAccount)) ||
        !chartOfAccounts.some((account) => parseInt(account.accountCode) === parseInt(creditAccount))
      ) {
        return {
          error: true,
