...
export function getLinkFetchTableWithConds({table, conds, values, useOnline}){
    const baseUrlFmt = useOnline? baseUrl_Prod : baseUrl;
    ...
    return `${baseUrlFmt}/server.php/api/fetch?db=${domain}&t=${table}&c=${conds}&v=${values}`; //eg c=userId,phoneNo&v=DEMO@ben.ted,08060000
}
...
export function getImageLink(img, isLogo){
    const img1 =  `${baseUrl_Prod}/image_server.php?image=${img}&timestamp=${Date.now()}`;
    const img2 =  `${baseUrl_Prod}/image_server.php?image=${img}&isLogo=TRUE&timestamp=${Date.now()}`;
    return isLogo? img2 : img1;
}
...
export function getPdfServer(db){
    const url = "https://phpserver.gofamintpsogba.org/servePdfReport.php?db="+db;
    return url
}
export function getCoyLogoPath(domain){
    const url = `https://quickrecords.gofamintpsogba.org/image_server.php?image=${domain?.toUpperCase()}@LOGO&isLogo=TRUE`;
    return url
}
