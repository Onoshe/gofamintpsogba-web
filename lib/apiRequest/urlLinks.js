...
export function getLinkFetchTable({table, domain}){
    return `${baseUrl}/server.php/api/fetch?db=${domain}&t=${table}`;
}
export function getLinkFetchTableWithConds({table, conds, values}){
    const domain = table.split("_")[0];
...
export function getLinksAdmin(domain){
    const usersAccountUrl = `${baseUrl}/server.php/api/users-account?db=${domain}&t=demo_usersaccount`;
    const clientsDataUr = `${baseUrl}/server.php/api/fetch?db=${domain}&t=_clients`;
    const accessDataUrl = `${baseUrl}/server.php/api/fetch?db=${domain}&t=_access&s=id,name,slug,description`;
    const dbTablesUrl = `${baseUrl}/server.php/api/show?db=${domain}&t=_clients&s=dtables&db=${db_}`; //t must be a valid table in the db
    const accessUrl = `${baseUrl}/server.php/api/fetch?db=${domain}&t=_access`;
    const settingsUrl = `${baseUrl}/server.php/api/fetch?db=${domain}&t=_settings`;
    const subscriptionsUrl = `${baseUrl}/server.php/api/fetch?db=${domain}&t=_subscriptions`;
    const backupUrl = `${baseUrl_Prod}/devQuery.php?db=${domain}`; //While backup csv works on both localhost and online, backup msl only works on online
    return {usersAccountUrl, clientsDataUr, accessDataUrl, dbTablesUrl, accessUrl, subscriptionsUrl, settingsUrl, backupUrl}
}
...
export function getLinkAdminCreateClient({form, domain}){
    return `${baseUrl}/server.php/api/fetch?db=${domain}&t=_clients&c=companyDomain&v=`+form.companyDomain
}
...
export function getLinkPostTrans(domain){
    return {patch:`${baseUrl}/server.php/api/patch-trans?db=${domain}`, post:`${baseUrl}/server.php/api/post-trans?db=${domain}`}
}
...
export function getLinkFindUser({domain, form}){
    return `${baseUrl}/server.php/api/fetch?db=${domain}&t=${domain}_usersaccount&c=userId&v=${form.userName}`
