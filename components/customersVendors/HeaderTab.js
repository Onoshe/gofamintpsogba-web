...
const TabWrapper = ({children, classNameCont, activeTab, handleActiveTab, tab1Name, tab2Name, tab1Title, tab2Title, subTitle, hideIcon}) => {
  ...
  return (
      <div className={classNameCont}>
          <div className='px-4 py-2 flex flex-col bg-[aliceblue] border border-b-[#e4f1fd] smc:flex-row smc:items-center justify-between'>
            <p className='text-lg font-bold text-blue-700'>{activeTabTitle}</p>
            <div className='flex self-end gap-3 flex-col xsmc:flex-row'>
                <button className={`btn btn-sm ${activeTab === tab1Name?  'btn-accent text-white' : 'bg-gray-300 text-black hover:bg-gray-300'}`}
                  onClick={()=>handleActiveTab(tab1Name)}>
                  {tab1Title}
                </button>
                <button className={`btn btn-sm ${activeTab === tab2Name? 'btn-info text-white' :'bg-gray-300 text-black hover:bg-gray-300'} `} 
