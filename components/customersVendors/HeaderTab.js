  ...
  return (
      <div className={classNameCont}>
          <div className='p-4 flex flex-col bg-[aliceblue] border border-b-[#e4f1fd] smc:flex-row smc:items-center justify-between'>
            ...
            <div className='flex self-end gap-3 flex-col smc:flex-row'>
                <button className={`btn ${activeTab === tab1Name?  'btn-accent text-white' : 'bg-gray-300 text-black hover:bg-gray-300'}`}
                  onClick={()=>handleActiveTab(tab1Name)}>
                  {tab1Title}
                </button>
                <button className={`btn ${activeTab === tab2Name? 'btn-info text-white' :'bg-gray-300 text-black hover:bg-gray-300'} `} 
