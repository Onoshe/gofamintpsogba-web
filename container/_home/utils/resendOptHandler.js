...
import { getLinkPostUser, getLinkResend } from "@/lib/apiRequest/urlLinks";



const  resendOtpHandler = async (form, setAlert, setModalAlert, setModalAlertCall)=>{
        const domain = form?.userName?.split('@')[0].toLowerCase();
        const urlLink = getLinkResend({domain, form});
