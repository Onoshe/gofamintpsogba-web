...
const ReportTable = ({
    ...
    clickableHeader, onClickHeader, clickableRow, onClickRow,  clickableRowCellKeys,
     onClickRowCell,  classNameTable,  classNameHeaderTR, classNameRowsTR}) => {
        ...
        <table className={`table table-sm table-zebra ${pinRow? 'table-pin-rows' :''} ${pinCol? 'table-pin-cols' :''}`}>
            ...
            <tbody>
                {rows?.map((row, i)=>{
                    //console.log(row)
                    return(
                        <tr key={`${i}td`} className={`${classNameRowsTR} ${row?.rowStyle} bg-sky-100`}>
                            <td className={``}>{i+1}</td>
                            {
                               rowKeys?.map((key, id)=>{
