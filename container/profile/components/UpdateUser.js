    ...
    <div className='z-50 fixed top-0 bottom-0 right-0 w-full h-screen bg-[#33486ea7] flex justify-center items-center'>
        <div className='relative bg-white boder border-blue-500 p-3 rounded-md w-full max-w-[450px]'>
            <MdClose size={28} className='absolute right-5 cursor-pointer text-red-500 hover:text-red-800'
             onClick={handleClose}/>
            <p className='text-center font-bold py-3'>Update User</p>
            <form className='flex flex-col gap-3 p-4' onSubmit={(e)=>handleEditUser(e, updateForm)}>
                <div>
                    <p>Username</p>
                ...
                {!deleteUser?.delete && <button className='btn btn-info' type='submit'>Save Changes</button>}

                <HorizontalLine widths={100} margTop={20} margBot={10} bColor={'red'}/>
                
            </form>
                <div className='flex flex-row p-4 gap-5 text-sm'>
                    <div className={`${deleteUser?.delete? '' : 'hidden'}`}>
                        ...
                        <input name="passwordDelete" value={updateForm.passwordDelete} className='bg-red-50 rounded-sm p-2 my-3 border border-red-400 outline-none' placeholder='Login password' required  onChange={onChangeHandler}/>
                        <div className='flex flex-row gap-8 flex-wrap'>
                            {deleteUser?.delete? 
                             <p className='btn btn-error' onClick={handleDeleteUserContinue}>{"Continue"}</p>
                            :<p className='btn btn-error' onClick={(e)=>handleDeleteUser(e, "CONTINUE", updateForm)}>{"Delete User"}</p>
                            }
                            <p className='btn bg-gray-400' onClick={(e)=>handleDeleteUser(e, "CANCEL", updateForm)}>Cancel</p>
                        </div>
                    </div>
                    <p className={`btn btn-error ${deleteUser?.delete? 'hidden' : ''}`} onClick={(e)=>handleDeleteUser(e, "DELETE", updateForm)}>Delete User</p>
                </div>
        </div>
    </div>
  )
